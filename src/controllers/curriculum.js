...
const enviardatos = async (req, res) => {
  console.log(`Se acaba de ingresar ${req.body.name}`);

  const gustos = req.body.gustos.split(',');
  req.body.gustos = gustos;

  const result = await mongodb.getDb().db('Test').collection('curriculum').insertOne(req.body);
